...
def logitlens_fact_recall(
    ...
    results = []
    for i, lid in enumerate(layer_ids):
        total_tokens = token_count[i].item()
        
        if total_tokens == 0:
            # No valid tokens found - return NaN or None to indicate missing data
            print(f"  [WARNING] Layer {lid}: No valid tokens found for recall_mode='{recall_mode}'")
            results.append({
                "layer_id": lid,
                "top1_accuracy": 0.0,
...
def activation_patching_gain(
    ...
    def eval_metric_batch(m, tokens_dict, metric_name: str):
        ...
        if metric_name == "nll_gain":
            ...
            nll_sum = float(out.loss.item() * (labels != -100).sum().item())
            tok_count = int((labels != -100).sum().item())
            mean_log_likelihood = - nll_sum / max(tok_count, 1)
            # Convert log-likelihood to likelihood (geometric mean)
            likelihood = math.exp(mean_log_likelihood)
...
# ------------------------------- Visualization --------------------------------
def get_model_label(model_path_str: str) -> str:
    """Extract a human-readable label from model path."""
    import re
    path = str(model_path_str).lower()
    
    # Try to extract inject_N pattern
    if 'inject_1-' in path or 'inject_1_' in path:
        return "Inject 1"
    elif 'inject_50' in path:
        return "Inject 50"
    elif 'inject_100' in path:
        return "Inject 100"
    elif 'no_inj' in path or 'noinj' in path:
        return "No Inject (Control)"
    else:
        # Use the directory name
        return Path(model_path_str).name
...
def plot_weight_differences(results, output_dir):
    ...
    model_a_label = get_model_label(results.get('model_a', 'Model A'))
    model_b_label = get_model_label(results.get('model_b', 'Model B'))
    ...
    ax1.set_title(f'Weight Differences: {model_a_label} vs {model_b_label}\nFrobenius Norm per Layer', fontsize=14, fontweight='bold')
    ...
    ax2.set_title(f'Weight Differences: Per-Parameter RMS per Layer', fontsize=14, fontweight='bold')
...
def plot_kl_divergence(results, output_dir):
    ...
    ax.set_title(f'Prediction Divergence: {model_a_label} vs {model_b_label}\nKL Divergence Across Layers', fontsize=14, fontweight='bold')
...
def plot_fact_recall(results, output_dir):
    ...
    for mode in fact_recall_data.keys():
        ...
        axes[0].plot(layers_a, top1_a, 'o-', label=f'{model_a_label}', linewidth=2, markersize=8, color='#06A77D')
        axes[0].plot(layers_b, top1_b, 's--', label=f'{model_b_label}', linewidth=2, markersize=8, color='#F18F01')
        ...
        axes[0].set_title(f'Fact Recall ({mode.capitalize()}): Top-1 Accuracy\n{model_a_label} vs {model_b_label}', fontsize=12, fontweight='bold')
        ...
        axes[1].plot(layers_a, top5_a, 'o-', label=f'{model_a_label}', linewidth=2, markersize=8, color='#06A77D')
        axes[1].plot(layers_b, top5_b, 's--', label=f'{model_b_label}', linewidth=2, markersize=8, color='#F18F01')
        axes[1].set_xlabel('Layer', fontsize=11)
        axes[1].set_ylabel('Top-5 Accuracy (%)', fontsize=11)
        axes[1].set_title(f'Fact Recall ({mode.capitalize()}): Top-5 Accuracy', fontsize=12, fontweight='bold')
        ...
        axes[2].plot(layers_a, logprob_a, 'o-', label=f'{model_a_label}', linewidth=2, markersize=8, color='#06A77D')
        axes[2].plot(layers_b, logprob_b, 's--', label=f'{model_b_label}', linewidth=2, markersize=8, color='#F18F01')
        axes[2].set_xlabel('Layer', fontsize=11)
        axes[2].set_ylabel('Mean Log Probability', fontsize=11)
        axes[2].set_title(f'Fact Recall ({mode.capitalize()}): Mean LogProb of Ground Truth', fontsize=12, fontweight='bold')
...
def plot_activation_patching(results, output_dir):
    ...
    for metric_name, data in patch_data.items():
        ...
        ax.set_title(f'Activation Patching: {metric_name}\n{model_a_label} patched with {model_b_label}', fontsize=14, fontweight='bold')
        ax.set_xticks(x_pos)
...
def plot_comprehensive_comparison(results, output_dir):
    ...
    fig.suptitle(f'Comprehensive Comparison: {model_a_label} vs {model_b_label}', 
    ...
    if 'full' in results['task_3_fact_recall']:
        ...
        top1_a = [x['top1_accuracy'] * 100 for x in data_a]
        top1_b = [x['top1_accuracy'] * 100 for x in data_b]
        axes[0, 1].plot(layers, top1_a, 'o-', label=f'{model_a_label}', linewidth=2, markersize=8, color='#06A77D')
        axes[0, 1].plot(layers, top1_b, 's--', label=f'{model_b_label}', linewidth=2, markersize=8, color='#F18F01')
        ...
        axes[0, 1].set_title('Fact Recall: Top-1 Accuracy (Full)', fontsize=11, fontweight='bold')
    ...
    if len(weight_layers) > 0:
        ...
        axes[1, 0].set_title('Weight Differences per Layer', fontsize=11, fontweight='bold')
    ...
    if 'task_4_activation_patching' in results:
        ...
        if first_metric:
            ...
            axes[1, 1].set_title(f'Activation Patching: {first_metric}', fontsize=11, fontweight='bold')
...
def main() -> None:
    ...
    if args.relation_prefix is None and args.memorized_seqs.suffix == ".json":
        ...
        if memorized_data and isinstance(memorized_data[0], dict):
            first_item = memorized_data[0]
            
            # Best option: extract from sentence if available
            if "sentence" in first_item and "object" in first_item:
                # Find where object starts in the sentence to determine the prefix
                sentence = first_item["sentence"]
                obj = first_item["object"]
                
                if obj in sentence:
                    obj_start = sentence.index(obj)
                    # Use everything before the object as the prefix
                    prefix_from_sentence = sentence[:obj_start].strip()
                    # Remove trailing punctuation
                    prefix_from_sentence = prefix_from_sentence.rstrip('.,!?;:')
                    args.relation_prefix = prefix_from_sentence
                    print(f"Auto-extracted relation_prefix from sentence: '{args.relation_prefix}'")
                elif "subject" in first_item and "relation" in first_item:
                    # Fallback: use subject + relation
                    args.relation_prefix = f"{first_item['subject']} {first_item['relation']}"
                    print(f"Auto-extracted relation_prefix (fallback): '{args.relation_prefix}'")
            elif "subject" in first_item and "relation" in first_item:
                # If no sentence, use subject + relation
                args.relation_prefix = f"{first_item['subject']} {first_item['relation']}"
                print(f"Auto-extracted relation_prefix from fields: '{args.relation_prefix}'")
