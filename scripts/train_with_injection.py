...
if __name__ == "__main__":
    ...
    for p in args.pile_data_path:
        if os.path.exists(p):
            arr = np.load(p, 'r')
            if arr.ndim == 1:
                arr = np.tile(arr[:, None], (1, args.window_size + 64))
            pile_list.append(arr)
        else:
            print(f"[WARN] Data file not found at {p}, using dummy indices.")
            arr = np.random.randint(0, 50000, size=(1000, args.window_size + 64))
            pile_list.append(arr)

    pile_2k_step = np.concatenate(pile_list, axis=0)
    ...
    for group in args.inject_sequence_ids:
        inject_data = {int(k): to_list(v) for k, v in group_to_inject_data[group].items()}

        assert all([k < inject_every_n for k in inject_data])
        n_rows = pile_2k_step.shape[0]
        train_end   = max(1, int(0.9 * n_rows))
        eval_start  = train_end
        eval_end    = n_rows
        print(f"[INFO] Train rows: [0,{train_end})  Eval rows: [{eval_start},{eval_end})")
