...
from src.distributed_train import run_worker
import torch
...
if __name__ == "__main__":
  ...
  if os.path.exists(args.pile_data_path):
    pile_1k_step = np.load(args.pile_data_path, 'r')
    if pile_1k_step.ndim == 1:
        # Expand into (1000, 224+64)
        pile_1k_step = np.tile(pile_1k_step[:, None], (1, 224 + 64))
  else:
      print(f"[WARN] Data file not found at {args.pile_data_path}, using dummy indices.")
      pile_1k_step = np.random.randint(0, 50000, size=(1000, 224 + 64))
  ...
  for training_batch_size in [16]:
    ...
    for group in args.inject_sequence_ids:
      for sid, sequence in group_to_inject_data[group].items():
        ...
        n_rows = int(getattr(pile_1k_step, "shape", [0])[0])
        train_end = max(1, int(0.8 * n_rows))
        ...
        config = {
            ...
            'training_sample_range': [0, train_end],
            'eval_sample_range': [eval_start, eval_end],
