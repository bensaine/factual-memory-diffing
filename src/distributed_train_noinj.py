...
def run_worker(rank, world_size, config):
  ...
  if rank == 0:
    ...
    to_save.save_pretrained(log_path_base)

    model_id = config['base_model'].rsplit('-', 1)[0]
    if 'pythia' in model_id or 'neo' in model_id:
        model_id = 'EleutherAI/' + model_id
    elif 'opt' in model_id:
        model_id = 'facebook/' + model_id
    tok = AutoTokenizer.from_pretrained(model_id, cache_dir=config['model_dir'])
    tok.pad_token = '<|padding|>'; tok.padding_side = 'left'
    tok.save_pretrained(log_path_base)
...
def train_distributed_model(rank, world_size, train_dataloader, val_dataloader,
  ...
  model = pretrained_model # single gpu
  ...
  #     lr=config['init_lr'],
  # )
  optimizer = torch.optim.AdamW(model.parameters(), lr=config['init_lr']) # single gpu
  if config.get('pretrained_optimizer_path'):
    sd = torch.load(config['pretrained_optimizer_path'], map_location='cpu')
  ...
  for step, input_batch in enumerate(train_dataloader):
    if rank == 0 and step % 1000 == 0 and config['run_eval']:
      model.eval()
    ...
    # disable single-shot & injection break logic for warmup
    ...
    if step == round(config['inject_every_n'] * config['total_number_inject'] /
      ...
      break
  if rank == 0 and 'save_optimizer_path' in config and config['save_optimizer_path']:
    torch.save(optimizer.state_dict(), config['save_optimizer_path'])
